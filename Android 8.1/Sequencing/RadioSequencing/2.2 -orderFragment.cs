...
//            using (var rs = new XMLRS($"Select GEO=dbo.CheckFlag(flags,'GEO'), COD3 from Servicios WHERE Codigo='{Values.gService}'", Values.gDatos))
//            {
//                await rs.OpenAsync();
//                if (rs.RecordCount != 0)
//                {
//                    Values.IsLocationService = rs["GEO"].ToInt() == 1;
//                    Values.COD3 = rs["COD3"].ToString();
//                    Values.sFt.LocationVisibility = true;
//                }
//                else
//                {
//                    Values.IsLocationService = false;
//                    Values.sFt.LocationVisibility = false;
//                }
//            }
//            // create GeoSession
//            Values.gDatos.DataBase = "GEO";
//            using (var sp = new SPXML(Values.gDatos, "GEO..pGeoSessionCabAdd"))
//            {
//                sp.AddParameterValue("COD3", Values.COD3);
//                sp.AddParameterValue("System", "LOGISTICA");
//                sp.AddParameterValue("Service", Values.gService);
//                sp.AddParameterValue("ServiceSession", Values.gSession);
//                sp.AddParameterValue("UserCode", Values.gDatos.User);
//                try
//                {
...
//                catch (Exception ex)
//                {
//                    Toast.MakeText(Activity, $"Failure starting GeoLocation: {ex.Message}", ToastLength.Long).Show();
//                    Values.iFt.pushInfo(ex.Message);
//                    Values.IsLocationService = false;
//                    //return false;
//                    //buttonOk.Enabled = true;
...
//            var _settings = new Settings { User = Values.gDatos.User, Password = Values.gDatos.Password, Block = Values.gBlock, Order = Values.gOrderNumber, Session = Values.gSession, Service = Values.gService, COD3=Values.COD3 };
...
//                try
//                {
//                    Activity.RunOnUiThread(async ()=> location = await Geolocation.GetLocationAsync(request));
//                }
//                catch (Exception ex)
//                {
...
//                using (var _rs = new XMLRS(string.Format("select numero,partnumber,qty,cajas,rack,Modulo from LOGISTICA..etiquetas where Numero_orden={0} and Tipo='PEQ'", Values.gOrderNumber), Values.gDatos))
//                {
//                    await _rs.OpenAsync();
